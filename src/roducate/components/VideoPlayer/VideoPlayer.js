    ...
    return (
        <div>
            <div className="video-player">
                <video style={{borderRadius: 5}} ref={videoRef} onPlay={() => dispatch(setIsPlaying(true))}
                       onPause={() => dispatch(setIsPlaying(false))} onLoadedMetadata={handleLoadedMetadata}
                       onTimeUpdate={() => setCurrentTime(videoRef.current?.currentTime)}>
                    <source src={`${process.env.REACT_APP_3BUCKET_URL}${store.mediaList[index]}`} type="video/mp4"/>
                ...
                {!isLoading && <div className="d-flex justify-content-between"
                    ...
                    <div className="controls">
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                            <span className="vp-time">{secondsToHMS(videoRef.current?.currentTime)}</span>
                            <input type="range" min="0" max={videoRef.current?.duration || 0}
                                   value={currentTime} step="1" onChange={e => handleProgressChange(e)}/>
                            <span>{secondsToHMS(videoRef.current?.duration || 0)}</span>
                        </div>
                    </div>
