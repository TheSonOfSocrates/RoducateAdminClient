...
import {AddPeriodModal} from "../../modals/AddPeriodModal/AddPeriodModal";
import {showAddPeriodModal} from "../../../redux/actions/modal";
...
const PeriodComp = () => {
    ...
    const loadData = async () => {
        ...
        try {
            response = await axios.post('/library/termsLevel/all', {
                pageNumber,
                pageSize,
                searchStr,
                sort
    ...
    const deleteData = async (ids) => {
        dispatch(showLoadingScreen(true))
        try {
            const res = await axios.post('/library/termsLevel/deleteTermsLevels', {ids})
            dispatch(showLoadingScreen(false))
            if (res.data.success) {
                await MySwal.fire({
                    title: 'Good job!',
                    text: 'You deleted period successfully!',
                    icon: 'success',
                    timer: 2000,
                    position: 'center',
                    customClass: {
    ...
    return (
        <Fragment>
            ...
            <AddPeriodModal period={editingRow} onUpdate={loadData}/>
        </Fragment>
    )
}
