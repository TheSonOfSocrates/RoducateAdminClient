...
import {showAddTopicModal} from "../../../redux/actions/modal";
...
const TopicComp = ({fetchData}) => {
    ...
    const loadRecentInfo = async () => {
        ...
        try {
            response = await axios.post('/library/topic/recently')
        } catch (e) {
    ...
    const loadData = async () => {
        ...
        let response = undefined
        try {
            response = await axios.post('/library/topic/getTopics', {
                pageNumber,
                pageSize,
                searchStr,
                sort
        ...
        if (response && response.data.success) {
            setTotalPages(response.data.topicList.totalPages)
            setDataList(response.data.topicList.data)
        }
    ...
    return (
        <Fragment>
            {recentDataList?.map((item, idx) => <div class="mr-2"><TopicCard topic={item}/></div>)}
            <Card className="mt-1">
            ...
            <CreateTopic topic={editingRow} onUpdate={loadData}/>
        </Fragment>
    )
}
