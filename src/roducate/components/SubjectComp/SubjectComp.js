...
import {showAddSubjectModal} from "../../../redux/actions/modal";
...
const SubjectComp = () => {
            ...
            cell: row => {
                return (
                    <div style={{display: 'grid'}}>
                        <span style={{marginBottom: 5}}>{row.title}</span>
                        <span>{row.topics.map((item) => item.title).join(',')}</span>
                    </div>
                )
            }
        },
    ...
    const [recentDataList, setRecentDataList] = useState(undefined)
    ...
    const loadRecentInfo = async () => {
        ...
        try {
            response = await axios.post('/library/subject/recently')
        } catch (e) {
        ...
        if (response) {
            setRecentDataList(response.data)
        }
    }
    ...
    const loadData = async () => {
        ...
        let response = undefined
        try {
            response = await axios.post('/library/getSubjects', {
                pageNumber,
                pageSize,
                searchStr,
                sort
        ...
        if (response && response.data.success) {
            ...
            setDataList(response.data.subjects.data)
        }
    ...
    return (
        <Fragment>
            <div class="d-flex mb-1" style={{overflowX: 'auto'}}>
                <div className="col-3 mr-1 align-items-center" style={{
                    height: '100%',
                    padding: '15px',
                    ...
                    <div className="row d-flex justify-content-between">
                        <div className="col-8 d-flex justify-content-between pr-0">
                            <span className="user-type-cnt">Notes</span>
                            <span className="user-type-count">{recentDataList?.totalMaterialData.noteCount}</span>
                        </div>
                    ...
                    <div className="row d-flex justify-content-between" style={{marginTop: '-20px'}}>
                        <div className="col-8 d-flex justify-content-between pr-0">
                            <span className="user-type-cnt">Videos</span>
                            <span className="user-type-count">{recentDataList?.totalMaterialData.videoCount}</span>
                        </div>
                        <div className="col-4 pl-0">
                            <XYPlot
                                width={120}
                    ...
                    <div className="row d-flex justify-content-between" style={{marginTop: '-20px'}}>
                        <div className="col-8 d-flex justify-content-between pr-0">
                            <span className="user-type-cnt">Podcasts</span>
                            <span className="user-type-count">{recentDataList?.totalMaterialData.podcastCount}</span>
                        </div>
                        <div className="col-4 pl-0">
                            <XYPlot
                                width={120}
                    ...
                    <div className="row d-flex justify-content-between" style={{marginTop: '-20px'}}>
                        <div className="col-8 d-flex justify-content-between pr-0">
                            <span className="user-type-cnt">Games</span>
                            <span className="user-type-count">{recentDataList?.totalMaterialData.gameCount}</span>
                        </div>
                        <div className="col-4 pl-0">
                            <XYPlot
                                width={120}
                ...
                {recentDataList?.subjects.map((item, idx) => <div class="mr-2"><SubjectCard subject={item}/></div>)}
            </div>
            <Card>
            ...
            <AddSubjectModal subject={editingRow} onUpdate={loadData}/>
        </Fragment>
    )
}
