...
import {AddLevelModal} from "../../modals/AddLevelModal/AddLevelModal";
import {showAddLevel} from '@store/actions/modal'
...
const LevelComp = ({fetchData}) => {
    ...
    useEffect(async () => {
        if (!store.isVisibleAddLevel)
            loadData()
    }, [store.isVisibleAddLevel, pageNumber, pageSize, sort])
    ...
    const loadData = async () => {
        ...
        try {
            response = await axios.post('/library/educationLevel/allLevelsWithPg', {
                pageNumber,
                pageSize,
                searchStr,
                sort
    ...
    const deleteData = async (ids) => {
        dispatch(showLoadingScreen(true))
        try {
            const res = await axios.post('/library/educationLevel/deleteLevels', {ids})
            dispatch(showLoadingScreen(false))
            if (res.data.success) {
                await MySwal.fire({
                    title: 'Good job!',
                    text: 'You deleted level successfully!',
                    icon: 'success',
                    timer: 2000,
                    position: 'center',
                    customClass: {
    ...
    return (
        <Fragment>
            ...
            <AddLevelModal currentLevel={editingRow} onUpdate={loadData}/>
        </Fragment>
    )
}
