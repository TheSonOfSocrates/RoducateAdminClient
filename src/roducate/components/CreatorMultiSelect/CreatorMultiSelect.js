...
import './CreatorMultiSelect.css'
import Avatar from "../../../@core/components/avatar";

export const CreatorMultiSelect = ({data, onSelected, filter = '', idField = '_id'}) => {
    ...
    const onItemSelected = (selectedItem) => {
        let tmpList = []
        const isExist = selectedDataList.some(item => item._id === selectedItem._id)
        if (isExist) {
            tmpList = selectedDataList.filter((item) => item._id !== selectedItem._id)
        } else {
            tmpList = [...selectedDataList, selectedItem]
        }
    ...
    return (
        <div style={{maxHeight: 300, overflow: 'overlay', marginBottom: 20}}>
            {data.map((item) => (item['firstName'] + item['lastName']).toLowerCase().includes(filter.toLowerCase()) &&
                <div>
                    <div className="d-flex justify-content-between mb-1 ml-1">
                        <div className="d-flex justify-content-center align-items-center">
                            <Avatar img={`${process.env.REACT_APP_3BUCKET_URL}${item.avatar}`} imgHeight='40'
                                    imgWidth='40' status='online'/>
                            <div className="ml-1" style={{display: 'grid'}}>
                                <span
                                    className="Creator-Multi-sub-select Creator-Multi-full-name">{item['firstName'] + ' ' + item['lastName']}</span>
                                <span className="Creator-Multi-role">{item['role'].join(', ')}</span>
                            </div>
                        </div>
