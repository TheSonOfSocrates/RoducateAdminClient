...
export const SingleSelect = ({
    ...
    return (
        <div style={{maxHeight: 300, overflow: 'overlay', marginBottom: 20}}>
            {data.length > 0 && data.map((item) => item[displayField].toLowerCase().includes(filter.toLowerCase()) &&
                <div className="mb-1 ml-1">
                    ...
                    {subShowingField1 !== '' &&
                        <span>{item[subShowingField1].title + ', ' + item[subShowingField2].title}</span>}
                </div>)}
            {data.length === 0 &&
