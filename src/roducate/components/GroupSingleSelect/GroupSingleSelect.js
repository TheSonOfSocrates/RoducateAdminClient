...
import './GroupSingleSelect.css'
import {Spinner} from "reactstrap";

export const GroupSingleSelect = ({selectedId, data, filter, onSelected}) => {
    ...
    return (
        <div style={{maxHeight: 300, overflow: 'overlay', marginBottom: 20}}>
            {data && data.length > 0 && data.map((item) => item.levels && item.levels.some(level => level[0].toLowerCase().includes(filter.toLowerCase())) &&
                <div>
                    <p className="SingleSelect-big-select">{item.title}</p>
                    <div className="mb-2">
                        {
                            item.levels && item.levels.map((level) => level[0].toLowerCase().includes(filter.toLowerCase()) &&
                                <div>
                                    <div className="d-flex justify-content-between mb-1 ml-1">
                                        <span className="SingleSelect-sub-select">{level[0]}</span>
                                        <div onClick={() => onItemSelected(level[0], level[1])}>
                                            {id !== level[1] &&
