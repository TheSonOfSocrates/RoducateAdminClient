...
import {showUploadModal} from "../../../redux/actions/modal";
...
const UploadPodcastComp = ({onRowSelected}) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            minWidth: '80px',
            cell: row => {
                return <div>{row.mediaURL.split('.')[1]}</div>
            }
        },
        {
            name: 'Size',
                ...
                return (
                    <div className='d-flex'>
                        ...
                        <Edit size={15} onClick={() => {
                            onRowSelected(dataList.find(item => item._id === row._id))
    ...
    useEffect(async () => {
        if (!store.showUploadModal)
            loadData()
    }, [store.showUploadModal, pageNumber, pageSize, sort])

    const loadData = async () => {
        dispatch(showLoadingScreen(true))
        let response = undefined
        try {
            response = await axios.post('/resource/podcastStore/getAll', {
                pageNumber,
                pageSize,
                searchStr,
                sort
        ...
        if (response && response.data.success) {
            setTotalPages(response.data.podcastStoreList.totalPages)
            setDataList(response.data.podcastStoreList.data)
        }
...
export default UploadPodcastComp