...
import {showAddTaskModal} from "../../../redux/actions/modal";
...
const TaskComp = ({fetchData}) => {
            ...
            cell: row => {
                return (
                    <div style={{display: 'grid'}}>
                        <span style={{marginBottom: 5}}>{row.topicTitle}</span>
    ...
    const loadData = async () => {
        ...
        try {
            response = await axios.post('/library/task/getAllTasks', {pageNumber, pageSize, searchStr, sort})
        } catch (e) {
    ...
    const deleteData = async (taskIds) => {
        dispatch(showLoadingScreen(true))
        try {
            const res = await axios.post('/library/task/deleteTask', {taskIds})
            dispatch(showLoadingScreen(false))
            if (res.data.success) {
                await MySwal.fire({
                    title: 'Good job!',
                    text: 'You deleted task successfully!',
                    icon: 'success',
                    timer: 2000,
                    position: 'center',
                    customClass: {
