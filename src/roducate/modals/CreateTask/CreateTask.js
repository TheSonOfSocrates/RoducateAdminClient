...
import {showAddTaskModal, showSubjectSelectModal, showTermsLevelSelectModal} from '@store/actions/modal'
...
export const CreateTask = ({
    ...
    const publish = async () => {
        if ((questions.length === 1 && !questions[0]?.questionContent) || !topicId || !subject || !termsLevel || ((duration.hours * 3600) + (duration.minutes * 60) + duration.seconds) === 0) {
            return MySwal.fire({
                title: 'Error!',
        ...
        let res;
        try {
            let link = '/resource/study/task'
            if (task)
                link = `/library/task/${task._id}/edit`
        ...
        if (res && res.data.success) {
            ...
            MySwal.fire({
                title: 'Good Job!',
                text: `Task ${task ? 'updated' : 'created'} successfully.`,
                icon: 'success',
                timer: 2000,
                position: 'center',
                customClass: {
    ...
    const getTopicCandi = async (level) => {
        const res = await axios.post('/library/task/availableTopics', {
            subjectId: subject?._id,
            termsLevel: level?._id
        })
        if (res && res.data.success) {
            setTopCands(res.data.topics.map((item) => ({value: item._id, label: item.title})))
        }
    }

    return (
        <Modal toggle={() => dispatch(showAddTaskModal())}
               isOpen={store.showAddTaskModal}
               style={{maxWidth: qaInsertMode ? '60%' : '80%'}}
               className="modal-dialog-centered"
        >
            <ModalBody toggle={() => dispatch(showAddTaskModal())}>
                <div className="d-flex justify-content-between align-items-center">
                    ...
                    <div style={{margin: 'auto'}} className="d-flex">
                        ...
                        {inlineMode && <Button className="mr-1" color='gradient-primary ghost'
                                               onClick={() => dispatch(showAddTaskModal())}>Cancel</Button>}
                    ...
                    <Button.Ripple onClick={() => dispatch(showAddTaskModal())} className='btn-icon rounded-circle'
                                   color='flat-danger'>
                        <X size={16}/>
                    </Button.Ripple>
                </div>
                ...
                <div className="row">
                    ...
                    {!qaInsertMode && <div className="col-4">
                        <div className="mt-2">
                            ...
                            {!inlineMode && <SelectOutlinedWrapper className="col-6" text="Period"
                                                                   onClick={() => {
                                                                       if (!subject)
                                                                           return MySwal.fire({
                                                                               title: 'Error!',
                            ...
                            {!inlineMode && <Select
                                ...
                                options={topCands}
                                isClearable={false}
                                value={topCands.find((item) => item.value === topicId)}
                                placeholder="Topic"
                                onChange={(item) => {
